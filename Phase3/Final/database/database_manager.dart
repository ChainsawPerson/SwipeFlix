    ...
    return openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(// Create Table `likedList` => LikedList
            "CREATE TABLE likedList (title_title_id INT PRIMARY KEY ASC ON CONFLICT REPLACE CONSTRAINT fk_likedList_title_id REFERENCES title_basics (title_id) NOT NULL, title_title_primaryTitle TEXT, like_timestamp TEXT );");
        await db.execute(// Create Table `dislikedList` => DislikedList
            "CREATE TABLE dislikedList (title_title_id INT PRIMARY KEY ASC ON CONFLICT REPLACE CONSTRAINT fk_dislikedList_title_id REFERENCES title_basics (title_id), title_title_primaryTitle TEXT, dislike_timestamp TEXT );");
        await db.execute(// Create Table `watchLaterList` => WatchList
            "CREATE TABLE watchLaterList (title_title_id INT PRIMARY KEY ASC ON CONFLICT REPLACE CONSTRAINT fk_watchLater_title_id REFERENCES title_basics (title_id) NOT NULL, title_title_primaryTitle TEXT, watchList_timestamp TEXT );");
      },
      version: _version,
    );
  }
...
// Add Movie to List via ID
  Future<void> addMovieToList(String list, int id) async {
    ...
    for (var movie in movies) {
      if (movie['title_id'] == id) {
        movieName = movie['title_primaryTitle'];
        break;
      }
    }
    ...
    await db.insert(
        ...
        {
          'title_title_id': id,
          'title_title_primaryTitle': movieName,
